...
- `lca_solve`: `POST` only.
  ...
  - snapshot family semantics: all three scopes reuse the same user-enhanced snapshot family, i.e. published data (`state_code=100..199`) plus the current user's private data
...
- `lca_query_results`: `POST` only.
  ...
  - all three scopes reuse the same user-enhanced snapshot family, where published data currently means `state_code=100..199`
